...
The following code plots the comparative time series plot with one y-axis of millions of individuals and second y-axis of fold-change of number of individuals from 2019-2022 for each ancestry compared to 2016-2019 (non-inclusive upper bound). 

```{r Comparative_Plot}
#Plotting time series plot with cumulative sum
p1 <- ggplot(expanded_cum_data)+
  geom_area(aes(x=as.Date(DATE), y=TIME_CUM/10^6, col =fct_rev(reorder(ANCESTRY, TIME_CUM)), fill = fct_rev(reorder(ANCESTRY, TIME_CUM))), position = 'stack')+
