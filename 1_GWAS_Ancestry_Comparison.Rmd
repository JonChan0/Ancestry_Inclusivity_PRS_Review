...
#Establishing order of ANCESTRY factor
ancestry_order <- expanded_cum_data %>%
  group_by(ANCESTRY) %>%
  summarise(ANCESTRY, max_cum_value = max(CUMULATIVE_INDIVIDUALS, na.rm=TRUE)) %>%
  distinct() %>%
  arrange(-max_cum_value)

expanded_cum_data <- expanded_cum_data %>%
  mutate(ANCESTRY = factor(ANCESTRY, levels=ancestry_order$ANCESTRY))
...
cumulative_plot <- ggplot(expanded_cum_data)+
  geom_area(aes(x=as.Date(DATE), y=TIME_CUM/10^6, fill = fct_rev(reorder(expanded_cum_data$ANCESTRY, expanded_cum_data$TIME_CUM))), position = 'stack', col = 'black')+
...
#ggsave('cumulative_plots.png', device='png',width=7, height=7, units = 'in', plot = cumulative_plot)
...
individual_period_data <- combined_period_data %>%
  select(1:3) %>%
  pivot_longer(c(2,3), names_to='Time_Period', values_to='Sum')
  
fold_change_plot <- ggplot(combined_period_data, aes(x='2019-2021\nvs.\n2016-2018', y= FOLD.CHANGE))+
  geom_boxplot(colour = 'black', fill='light grey', width = 1)+
  geom_point(aes(col = fct_rev(reorder(ANCESTRY, MAX.CUMSUM))), size = 2.5)+
...
combo_plot <- plot_grid(cumulative_plot, fold_change_plot, align='h', rel_widths = c(0.8,0.2))
print(combo_plot)
ggsave('combo_plot.png',device='png',height = 7, width = 7, units='in', combo_plot)


plot_list <- list(cumulative_plot, fold_change_plot)
